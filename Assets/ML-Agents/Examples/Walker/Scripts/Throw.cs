...
public class Throw : MonoBehaviour
{
    ...
    {
        cam = Camera.main;
        mamAgent = FindObjectOfType<MamAgent>();
        itemRB = item.GetComponent<Rigidbody>();
        itemCol = item.GetComponent<Collider>();
        //audioSourceSFX = gameObject.AddComponent<AudioSource>();
        mamAgent.target = returnPoint;
    }
    ...
    void StartSwipe()
    {
        ...
        if (!mamAgent.returningItem)
        {
            mamAgent.target = item;
        }
    }
    ...
    public void PickUpItemGame()
    {
        ...
        item.position = mamAgent.mouthPosition.position; //Set stick position
        item.rotation = mamAgent.mouthPosition.rotation; //Set stick rotation
        item.SetParent(mamAgent.mouthPosition); //Parent the stick to the dog's mouth
        mamAgent.runningToItem = false; //We are no longer running towards the stick

        //The stick is now in the dog's mouth so we need to change
        //the dog's target to the return point
        mamAgent.target = returnPoint; //set the dog's target to the return point
        mamAgent.UpdateOrientationObjects(); //update the direction vector
        mamAgent.returningItem = true; //we are not returning the stick
    }
    ...
    public void DropItemGame()
    {
        ...
        mamAgent.returningItem = false; //We are done returning the stick
    ...
    public IEnumerator GoGetItemGame()
    {
        ...
        //GO GET THE STICK
        mamAgent.target = item; //Set the target to the stick.
        mamAgent.runningToItem = true; //We are now running towards the stick

        //Wait till we are in range of the stick
        while (mamAgent.m_WorldDirToWalk.sqrMagnitude > 5f) //wait until we are close
        {
            yield return null;
        }
        ...
        while (mamAgent.m_WorldDirToWalk.sqrMagnitude > 3f) //wait until we are close
        {
            yield return null;
        }
